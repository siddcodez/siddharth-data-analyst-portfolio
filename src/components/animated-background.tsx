...
import { Section, getKeyboardState, SPLINE_SKILL_MAPPING } from "./animated-background-config";
import { useSounds } from "@/hooks/use-sounds";

gsap.registerPlugin(ScrollTrigger);

const getSkillFromTarget = (name: string): Skill | undefined => {
  const normalized = name.toLowerCase();
  const skillKey = SPLINE_SKILL_MAPPING[normalized] || (normalized as SkillNames);
...
const AnimatedBackground = () => {
  ...
  // Tablet: 768px–1023px gets its own keyboard positions
  ...
  const handleMouseHover = (e: SplineEvent) => {
    ...
    if (e.target.name === "body" || e.target.name === "platform") {
      ...
      // Map Spline keycap name to new Data Analytics skill
      const mappedSkillName = SPLINE_SKILL_MAPPING[e.target.name.toLowerCase()];
      const skill = mappedSkillName ? SKILLS[mappedSkillName] : SKILLS[e.target.name as SkillNames];
  ...
  const getKeycapsAnimation = () => {
    ...
    const splineKeycapNames = Object.values(SKILLS).map((skill) => skill.name);

    const start = () => {
      removePrevTweens();
      Array.from(new Set(splineKeycapNames))
    ...
    const stop = () => {
      removePrevTweens();
      Array.from(new Set(splineKeycapNames)).forEach((keyName) => {
        const keycap = splineApp.findObjectByName(keyName);
        if (!keycap) return;
        const t = gsap.to(keycap.position, {
          y: 0,
  ...
  // Handle keyboard text visibility based on section (Dark mode only)
  useEffect(() => {
    if (!splineApp) return;
  ...
  return (
    <Suspense fallback={<div>Loading...</div>}>
      <Spline
        ...
        scene="/assets/skills-keyboard-data-science.spline?v=20260827-palette-final"
      />
    </Suspense>
  );
};
